...
-   The relationship between white poverty and Trump support seems to be positive. This relationship also looks reasonably linear.
:::
...
-   It's unreasonable to conclude that social policies increasing white poverty with cause an increase in Trump support, because there are likely confounding variables that influence both poverty and Trump support.
:::
...
-   At 6% white poverty, 43.161% Trump support is expected.
:::
...
-   It's a bad idea to predict Trump support at 85% white poverty, because 85% poverty is way outside the range of data.
:::
...
ggplot(data = trump, aes(y = trump_support, x = non_white)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Percentage of non-white identifiers",
       y = "Percentage of voters supporting Trump",
       title = "Trump support and non-white population") +
  theme_bw()
```
:::
...
-   I expect the correlation coefficient to be negative.
...
-   For intercept - When non-white population is at 0% the model predicts 58.33% Trump support.

-   For slope - For every increase in non-white population of 1%, there is a decrease in -.269% Trump support.
:::
...
-  Using the least squares regression line of Trump support vs. Non-white, a region with 70% humans identifying as people of color has 39.49% support of Trump.
:::
...
# Type your code and comments inside the code chunk
trump |>
  summarize(poverty_cor = cor(trump_support, poverty),
            non_white_cor = cor(trump_support, non_white))
```

-   Since poverty has a stronger correlation with Trump support, it's a better explanatory variable for Trump support.
:::
...
**A.** She'd predict 41.912% Trump support in a region with 61% of non-white people.

```{r}
# Type your code and comments inside the code chunk
results <- get_regression_points(nw_mod)
kable(head(results))
```

**B.** Her prediction would be an overestimate by 8.912%.

**C.** The residual is -8.912.
:::
